...
<div class="container" transition:scale|local={{ start: 0.7, delay: 500 }}>
	{#if currentUserData.id}
		<h1>
			{currentUserData.full_name ? currentUserData.full_name : currentUserData.phone_number} profile
		</h1>
	{/if}
	...
	<div class="user" transition:scale|local={{ start: 0.3 }}>
		<div class="text">
			<input
				aria-label="User's phone_number"
				type="text"
				placeholder="User's phone_number"
				name="phone_number"
				value={currentUserData.phone_number}
			/>
			<button
				class="save"
				aria-label="Save user's phone_number"
